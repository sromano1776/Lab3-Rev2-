...
public class Hand {
	...
	public Hand() {
		...
		NbrJoker = new NbrJoker();
	}

	private static int getNbrJoker() {
		return NbrJoker;
	}

	private static void setNbrJoker(int nbrJoker) {
		NbrJoker = nbrJoker;
	}
	...
	private int NbrJokers(ArrayList<Card> cards) {
		for (Card c : cards) {
	...
	private static boolean isJokerFlush(ArrayList<Card> cards, HandScore hs) {
		int check = cards.get(0).getiCardNbr();
		for (int x = 1; x <= (4 - NbrJoker); x++) {
			if (check != cards[x].geteSuit().getiSuitNbr()) {
				return false;
			}
		}
	...
	public static boolean isHandJokerStraight(ArrayList<Card> cards, HandScore hs) {
		int high = cards.get(4 - NbrJoker).geteRank().getiRankNbr();
		int low = cards.get(0).geteRank().getiRankNbr();
		int dist = high - low;
		if (dist <= 4) {
			for (int x = 1; x <= (4 - NbrJoker); x++) {
				if (low == cards.get(x).geteRank().getiRankNbr()) {
					return false;
				}
			}
	...
	public static boolean isHandJokerRoyalFlush(Hand h, HandScore hs, ArrayList<Card> cards) {
		if (isJokerFlush(h.getBestCardsInHand(), hs)) {
			int high = cards.get(4 - NbrJoker).geteRank().getiRankNbr();
			int low = cards.get(0).geteRank().getiRankNbr();
			int dist = high - low;
			if ((high <= 14) && (low >= 10) && (dist <= 4)) {
	...
	public static boolean isHandJokerStraightFlush(Hand h, HandScore hs) {
		if ((isHandJokerStraight(h.getCardsInHand(), hs)) && (isJokerFlush(h.getCardsInHand(), hs))) {
			hs.setHandStrength(eHandStrength.StraightFlush.getHandStrength());
			return true;
		} else {
			return false;
	...
	public static boolean isHandJokerFourOfAKind(Hand h, ArrayList<Card> cards, HandScore hs){
		
		int high = cards.get(4-NbrJoker).geteRank().getiRankNbr();
		int low = cards.get(0).geteRank().getiRankNbr();
		int dist = high-low;
		if ((NbrJoker==1)&&(isHandThreeOfAKind(h, hs))){
			hs.setHandStrength(eHandStrength.FourOfAKind.getHandStrength());
			return true;
		}
		else if ((NbrJoker==2)&&(isHandPair(h, hs))){
			hs.setHandStrength(eHandStrength.FourOfAKind.getHandStrength());
			return true;
		}
		else if((NbrJoker==3)){
			hs.setHandStrength(eHandStrength.FourOfAKind.getHandStrength());
			hs.setHiHand(h.getCardsInHand().get(dist).geteRank().getiRankNbr());
			return true;
		}
		else{
			return false;
	...
	public static boolean isHandJokerFullHouse(ArrayList<Card> cards, Hand h, HandScore hs){
		int high = cards.get(4-NbrJoker).geteRank().getiRankNbr();
		int low = cards.get(0).geteRank().getiRankNbr();
		int dist = high-low;
		if ((isHandPair(h, hs))&&(NbrJoker==2)){
			hs.setHandStrength(eHandStrength.FullHouse.getHandStrength());
			hs.setHiHand(h.getCardsInHand().get(eCardNo.ThirdCard.getCardNo()).geteRank().getiRankNbr());
			return true;
		}
		else if ((isHandTwoPair(h, hs))&&NbrJoker==1){
			hs.setHandStrength(eHandStrength.FullHouse.getHandStrength());
			hs.setHiHand(h.getCardsInHand().get(eCardNo.ThirdCard.getCardNo()).geteRank().getiRankNbr());
			return true;
		}
	...
	public static boolean isHandJokerPair(Hand h, HandScore hs) {
		hs.setHandStrength(eHandStrength.Pair.getHandStrength());
		hs.setHiHand(h.getCardsInHand().get(eCardNo.FourthCard.getCardNo()).geteRank().getiRankNbr());
		return true;
